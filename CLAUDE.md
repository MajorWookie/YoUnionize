...
- **Cross-runtime shared prompts**: `/ask`, `/company-personalize`, and `/compensation-fairness` instantiate `Anthropic` directly (Deno can't import Bun packages) but they no longer inline their prompts — they import from `supabase/functions/_shared/prompts/`, which is a verbatim mirror of `packages/ai/src/prompts/{rag-answer,what-this-means,compensation-analysis}.ts`. **Workflow:** edit the package source, run `bun run prompts:generate`, commit both files. CI fails (`bun run prompts:check`) if the mirror drifts. Mirror files start with an `AUTO-GENERATED — DO NOT EDIT` banner.
- **Fairness scale**: `compensation-analysis.ts` emits `fairness_score` on a 1–100 scale with banded guidance, matching the `AnalysisData` interface in `src/routes/my-pay.tsx` and `FairnessGauge`'s 80/60/40 thresholds. A short-lived deploy emitted 1–10 with a different JSON shape; saved rows from that period are invalid — run `scripts/invalidate-compensation-analyses.ts --apply` once per environment to clear them.
